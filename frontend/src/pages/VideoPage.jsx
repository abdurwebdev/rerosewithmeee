...
const VideoPage = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const res = await axiosAuth.get(`/api/user/post/${postId}`);
    ...
    const fetchRecommended = async () => {
      try {
        const res = await axiosAuth.get(`/api/user/posts`);
  ...
  const handleFollowToggle = async () => {
    if (!post?.user?._id) {
      console.error("⚠️ No followUserId available", post?.user);
      return;
    }

    try {
      console.log("👉 Sending followUserId:", post.user._id);
          ...
          <div className="mt-5">
            ...
            {post.comments?.length > 0 ? (
              [...post.comments].reverse().map((c) => (
                <div
                  key={c._id}
                  className="border-b border-[#FF6A00] pb-2 mb-2 text-sm"
                >
          ...
          <div className="bg-zinc-900 text-white rounded-xl w-[90%] max-w-md p-5">
            ...
            <div className="flex justify-end gap-3 mt-5">
              <button
                ...
                className="px-3 py-2 rounded bg-zinc-700"
              >
                Cancel
              </button>
              <button
